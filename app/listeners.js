import { archiveNote, editNoteReducer, getArchivedFlag, unArchiveNote } from './store.js';
...
export const disableInputsListener = (listItem) => {
  ...
  editBtn.addEventListener('click', (e) => {
    const noteInputs = Array.from(e.target.closest('.notes__item').getElementsByClassName('note__text'));
...
export const watchInputChangesListener = (listItem) => {
  const inputsList = getEditableInputsList(listItem.id);
  inputsList.forEach((input) => {
    ...
    input.addEventListener('focusout', (e) => { 
      const editedInputName = e.target.classList[1].replace('text__', '');
      action.editedField = { [editedInputName]: e.target.value };
