import { archiveHandler, editNoteHandler, watchInputChangesHandler, addHandlers } from './handlers.js'

export const renderNoteList = (store)=>{
    ...
    store.listData.forEach((listItem)=>{

        if(listItem.isArchived == false) {
            // Get the right path of icon for this note
           let iconPath = getIconPath(store.noteIconPath, listItem.category)
        ...
        noteItem.innerHTML = `<div class="note__name">
                                <img class="note__icon" src="${iconPath}" alt="${listItem.category}">
                                <input class="note__text text__name" type="text" value="${listItem.name}">
                                      
                              </div>
                              <input class="note__text text__createdDate" type="text" value="${listItem.createData}">
                              <input class="note__text text__category" type="text" value="${listItem.category}">
                              <input class="note__text text__content" type="text" value="${listItem.content}">
                              <input class="note__text text__schedule" type="text" value="${listItem.schedule}">
                              <div class="note__controls controls">

                                <svg class="controls__editBtn"
                                ...
                                <svg class="controls__archiveBtn"
    ...
    renderNoteStatistic(store)
}   

const renderNoteStatistic = (store)=>{
    let statisticBlock = document.querySelector('#statisticList');
    const statisticBlockLength = Array.from(statisticBlock).length
    ...
    store.categoriesList.forEach(category => {

        store.listData.forEach((listEl) => {

            if(listEl.category == category ) {
                currentIterateCategory = listEl.category
                statisticIconPath = getIconPath(store.noteIconPath, category)

                if(listEl.isArchived == true) {
                    archivedNoteCount = archivedNoteCount + 1
                    console.log(archivedNoteCount)
                } else {
...
const getIconPath = (iconPaths, iconType)=>{
    let iconPathStr = '';
    for(let key in iconPaths) {
        ...
        let regExp = new RegExp(`${iconType}`,'gi')
